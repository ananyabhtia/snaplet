const ButtonBox = ({ handleSave, handlePrev, handleNext, handlePDF, currentStep }) => {
    ...
    return (
        <div className="flex flex-row w-full h-1/12 border-yellow-700 rounded-xl justify-between items-center mb-1 px-2">
            <div className="flex flex-row border-orange-700 rounded-xl justify-between items-center gap-1 flex-wrap">
                <div id="prev" onClick={!isButtonDisabled ? handlePrev : undefined} className={`flex flex-row font-bold py-1 px-2 sm:py-2 sm:px-4 rounded-full h-full items-center text-xs sm:text-sm ${isButtonDisabled ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-purple-300 hover:bg-purple-500 active:bg-purple-800 cursor-pointer'}`}>
                    <i className="fa-solid fa-arrow-left mr-1 sm:mr-2 text-xs sm:text-sm" />
                    <p>prev</p>
                </div>
                <div id="next" onClick={handleNext} className="flex flex-row bg-purple-300 hover:bg-purple-500 active:bg-purple-800 font-bold py-1 px-2 sm:py-2 sm:px-4 rounded-full h-full items-center text-xs sm:text-sm cursor-pointer">
                    <p>next</p>
