...
const VariableButton = ({ name, value, type, items }) => {
    ...
    switch (type) {
        case "variable":
            return (
                <div className="text-sm flex flex-row items-center bg-pink-200 hover:bg-pink-400 font-bold py-2 px-4 rounded-full border-2 h-10 m-2 w-95/100">
                    <p>name</p>
        ...
        case "frame":
            ...
            return (
                <div className="flex flex-col bg-blue-200 hover:bg-blue-400 font-bold py-2 px-4 rounded-full border-2 m-2 w-95/100">
                    <input onPointerDown={(e) => e.stopPropagation()} type="text" className="bg-white ml-2 mr-2 text-black w-2/5 rounded-full pl-2"></input>
                    <div ref={setNodeRef} className={`w-full h-25 bg-white border-black border-2 rounded-full mt-2 ${isOver ? 'ring-4 ring-blue-700' : ''}`}>
                        {items?.map((item) => 
